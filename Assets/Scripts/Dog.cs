...
public class Dog : MonoBehaviour
{
    ...
    void Update()
    {
        if(isAwake)
        {
            GetComponent<Animator>().SetBool("isAwake", true);
            RaycastHit2D hit_x;
            RaycastHit2D hit_y;

            hit_x = Physics2D.Raycast(new Vector2(transform.position.x, transform.position.y - 0.25f), Vector2.right * xDirection, 0.25f, raycastMask);
            Debug.DrawRay(new Vector2(transform.position.x, transform.position.y), Vector2.right * xDirection, Color.green);
            hit_y = Physics2D.Raycast(new Vector2(transform.position.x, transform.position.y - 0.25f), Vector2.up * yDirection, 0.25f, raycastMask);
